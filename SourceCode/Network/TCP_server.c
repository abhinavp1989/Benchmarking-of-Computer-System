...
# define file_size  671088640 //640 MB //Choose small file size in the multiples of 80 MB to run quickly in local machine
...
void *receive_data(void *input){
	...
	for(i = file_pos ; i<operation_div3 ; i+=8){
		...
		bytes_read = read( new_socket , buffer, 8);
    	bytes+=bytes_read;
...
int main(){
    ...
    if ((server_file_des = socket(AF_INET, SOCK_STREAM, 0)) == 0)
    {
    ...
    /*if (setsockopt(server_file_des, SOL_SOCKET, SO_REUSEADDR | SO_REUSEPORT,&opt, sizeof(opt)))
    {
        perror("setsockopt");
        exit(EXIT_FAILURE);
    }*/
    ...
    if (listen(server_file_des, 3) < 0)
    {
        perror("listen");
        exit(EXIT_FAILURE);
    }
    if ((new_socket = accept(server_file_des, (struct sockaddr *)&address, (socklen_t*)&addrlen))<0)
    {
        perror("accept");
        exit(EXIT_FAILURE);
    }
